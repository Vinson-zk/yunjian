...
* with ZK-Vinson. 
*
* @Title: ZKEurekaHttpClientFactory.java 
* @author Vinson 
* @Package com.zk.framwwork.serCen.eureka 
* @Description: TODO(simple description this file what to do.) 
* @date Jul 2, 2020 5:39:56 PM 
* @version V1.0 
*/
package com.zk.framwwork.serCen.eureka;
...
/** 
* @ClassName: ZKEurekaHttpClientFactory 
* @Description: TODO(simple description this class what to do.) 
* @author Vinson 
* @version 1.0 
*/
@SuppressWarnings("deprecation")
public class ZKEurekaHttpClientFactory implements TransportClientFactory {

    public static final String ZK_HTTP_X_DISCOVERY_ALLOW_REDIRECT = "ZK_X-Discovery-AllowRedirect";
...
//    @Deprecated
//    public ZKEurekaHttpClientFactory(EurekaJerseyClient jerseyClient, boolean allowRedirects) {
//        this(jerseyClient, null, -1,
//                Collections.singletonMap(ZK_HTTP_X_DISCOVERY_ALLOW_REDIRECT, allowRedirects ? "true" : "false"));
//    }
//
//    @Deprecated
//    public ZKEurekaHttpClientFactory(EurekaJerseyClient jerseyClient, Map<String, String> additionalHeaders) {
//        this(jerseyClient, null, -1, additionalHeaders);
//    }

    public ZKEurekaHttpClientFactory(ApacheHttpClient4 apacheClient, long connectionIdleTimeout,
            Map<String, String> additionalHeaders, ZKSerCenEncrypt zkSerCenEncrypt) {
        this(null, apacheClient, connectionIdleTimeout, additionalHeaders, zkSerCenEncrypt);
    }

    public ZKEurekaHttpClientFactory(EurekaJerseyClient jerseyClient, long connectionIdleTimeout,
            Map<String, String> additionalHeaders, ZKSerCenEncrypt zkSerCenEncrypt) {
        this(jerseyClient, null, connectionIdleTimeout, additionalHeaders, zkSerCenEncrypt);
    }

    private ZKEurekaHttpClientFactory(EurekaJerseyClient jerseyClient, ApacheHttpClient4 apacheClient,
            long connectionIdleTimeout, Map<String, String> additionalHeaders, ZKSerCenEncrypt zkSerCenEncrypt) {
    ...
    public EurekaHttpClient newClient(EurekaEndpoint endpoint) {
        return new ZKEurekaHttpClient(zkSerCenEncrypt, apacheClient, endpoint.getServiceUrl(), additionalHeaders);
    }
    ...
    public static ZKEurekaHttpClientFactory create(ZKSerCenEncrypt zkSerCenEncrypt,
            EurekaClientConfig clientConfig, Collection<ClientFilter> additionalFilters, InstanceInfo myInstanceInfo,
            AbstractEurekaIdentity clientIdentity, Optional<SSLContext> sslContext,
            Optional<HostnameVerifier> hostnameVerifier) {
        boolean useExperimental = "true"
                .equals(clientConfig.getExperimental("ZKEurekaHttpClientFactory.useNewBuilder"));

        ZKEurekaHttpClientFactoryBuilder clientBuilder = (useExperimental ? myExperimentalBuilder(zkSerCenEncrypt)
                : myNewBuilder(zkSerCenEncrypt)).withAdditionalFilters(additionalFilters)
                        .withMyInstanceInfo(myInstanceInfo).withUserAgent("Java-EurekaClient")
                        .withClientConfig(clientConfig).withClientIdentity(clientIdentity);
    ...
    public static ZKEurekaHttpClientFactoryBuilder myNewBuilder(ZKSerCenEncrypt zkSerCenEncrypt) {
        return new ZKEurekaHttpClientFactoryBuilder(zkSerCenEncrypt).withExperimental(false);
    }

    public static ZKEurekaHttpClientFactoryBuilder myExperimentalBuilder(ZKSerCenEncrypt zkSerCenEncrypt) {
        return new ZKEurekaHttpClientFactoryBuilder(zkSerCenEncrypt).withExperimental(true);
    }
    ...
    /**
     * Currently use EurekaJerseyClientBuilder. Once old transport in
     * DiscoveryClient is removed, incorporate EurekaJerseyClientBuilder here,
     * and remove it.
     */
    public static class ZKEurekaHttpClientFactoryBuilder
            extends EurekaClientFactoryBuilder<ZKEurekaHttpClientFactory, ZKEurekaHttpClientFactoryBuilder> {
        ...
        public ZKEurekaHttpClientFactoryBuilder(ZKSerCenEncrypt zkSerCenEncrypt) {
            this.zkSerCenEncrypt = zkSerCenEncrypt;
        }

        public ZKEurekaHttpClientFactoryBuilder withAdditionalFilters(Collection<ClientFilter> additionalFilters) {
            this.additionalFilters = additionalFilters;
            return this;
        }

        public ZKEurekaHttpClientFactoryBuilder withExperimental(boolean experimental) {
            this.experimental = experimental;
            return this;
        }

        @Override
        public ZKEurekaHttpClientFactory build() {
            Map<String, String> additionalHeaders = new HashMap<>();
            if (allowRedirect) {
                additionalHeaders.put(ZK_HTTP_X_DISCOVERY_ALLOW_REDIRECT, "true");
            }
        ...
        private ZKEurekaHttpClientFactory buildLegacy(Map<String, String> additionalHeaders, boolean systemSSL) {
            ...
            return new ZKEurekaHttpClientFactory(jerseyClient, connectionIdleTimeout, additionalHeaders,
                    this.zkSerCenEncrypt);
        }

        private ZKEurekaHttpClientFactory buildExperimental(Map<String, String> additionalHeaders) {
            ...
            return new ZKEurekaHttpClientFactory(apacheClient, connectionIdleTimeout, additionalHeaders, this.zkSerCenEncrypt);
        }
