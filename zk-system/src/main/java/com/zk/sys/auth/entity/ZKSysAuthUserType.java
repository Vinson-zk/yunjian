...
public class ZKSysAuthUserType extends ZKBaseEntity<String, ZKSysAuthUserType> {
    ...
    public static ZKSqlProvider initSqlProvider() {
        if(sqlProvider == null) {
            sqlProvider = new ZKSqlProvider(new ZKSqlConvertDelegating(), new ZKSysAuthUserType());
        }
        return sqlProvider;
    }
	...
	@ZKColumn(name = "c_user_type_id", isInsert = true, isUpdate = false, javaType = String.class, isQuery = true, queryType = ZKDBQueryType.EQ)
	String userTypeId;	
	/**
	 * 用户类型代码
	 */
	@Length(min = 0, max = 64, message = "{zk.core.data.validation.length.max}")
	@ZKColumn(name = "c_user_type_code", isInsert = true, isUpdate = false, javaType = String.class, isQuery = true, queryType = ZKDBQueryType.EQ)
	String userTypeCode;	
	/**
	 * 权限ID 
	 */
