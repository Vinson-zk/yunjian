...
public class ZKSysAuthRank extends ZKBaseEntity<String, ZKSysAuthRank> {
    ...
    public static ZKSqlProvider initSqlProvider() {
        if(sqlProvider == null) {
            sqlProvider = new ZKSqlProvider(new ZKSqlConvertDelegating(), new ZKSysAuthRank());
        }
        return sqlProvider;
    }
	...
	@ZKColumn(name = "c_rank_id", isInsert = true, isUpdate = false, javaType = String.class, isQuery = true, queryType = ZKDBQueryType.EQ)
	String rankId;	
	/**
	 * 职级代码
	 */
	@Length(min = 0, max = 64, message = "{zk.core.data.validation.length.max}")
	@ZKColumn(name = "c_rank_code", isInsert = true, isUpdate = false, javaType = String.class, isQuery = true, queryType = ZKDBQueryType.EQ)
	String rankCode;	
	/**
	 * 权限ID 
	 */
